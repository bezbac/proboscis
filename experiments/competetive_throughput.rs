...
fn simple_query(connection_url: &str) {
    ...
    let rows = client
        .query("SELECT array_to_string(ARRAY(SELECT chr((65 + round(random() * 25)) :: integer) FROM generate_series(1, 10000)), '')", &[])
        .unwrap();

    let text: &str = rows[0].get(0);
    assert_eq!(10000, text.len())
}
