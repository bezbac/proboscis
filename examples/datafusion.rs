...
use proboscis_resolver_datafusion::{
    datafusion::prelude::{CsvReadOptions, ExecutionContext},
    DatafusionResolver,
};
...
async fn run_proxy() -> String {
    let mut ctx = ExecutionContext::new();
    ctx.register_csv(
        "users",
        "examples/resources/csv/users.csv",
        CsvReadOptions::new(),
    )
    .unwrap();
    ...
    let mut proxy = Proxy::new(
        ...
        Box::new(DatafusionResolver::new(ctx)),
    );
...
async fn main() {
    ...
    let proxy_connection_url = run_proxy().await;
